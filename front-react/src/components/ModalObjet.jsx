  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <h2>{item.libelle}<p>#{item.id}</p></h2>
        <p><strong>Statut :</strong> {item.statut}</p>
        <p><strong>Date de mise en rayon :</strong>{dateFormatee}</p>
        <p><strong>Prix :</strong> {item.prix ? `${item.prix}€` : "-"}</p>
        <p><strong>Poids :</strong> {item.poids_kg ? `${item.poids_kg} kg` : "-"}</p>
